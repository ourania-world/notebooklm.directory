...
export function getAudioSources(audioPath) {
  ...
  const sources = [
    // 1. Local public folder as primary source
    `/${audioPath}`,
    // 2. Edge Function as secondary source (handles CORS and caching properly)
    `${supabaseUrl}/functions/v1/serve-audio?path=${encodeURIComponent(audioPath.replace(/^\//, ''))}`,
    // 3. Direct Supabase Storage URL as final fallback
    `${supabaseUrl}/storage/v1/object/public/audio/${encodeURIComponent(audioPath.replace(/^\//, ''))}`,
  ];
  
  // 4. Add original URL if it's a full URL
  if (audioPath.startsWith('http')) {
