...
export function getAudioUrl(audioPath) {
  ...
  // If it's a relative path, use the Edge Function with proper encoding
  ...
  // Try direct storage URL first as fallback
  const storageUrl = `${supabaseUrl}/storage/v1/object/public/audio/${encodeURIComponent(cleanPath)}`;
