...
export async function createCheckoutSession(userId, planId, successUrl, cancelUrl) {
  try {
    ...
    const response = await fetch('/api/create-checkout-session', {
      ...
      body: JSON.stringify({
        priceId: SUBSCRIPTION_PLANS[planId.toUpperCase()]?.stripeId || planId,
        successUrl,
        cancelUrl
      })
    })
...
export async function createCheckoutSessionEdgeFunction(userId, planId, successUrl, cancelUrl) {
  try {
