...
-- Add standard tier if it doesn't exist
INSERT INTO subscription_plans (id, name, description, price, interval, features, limits)
SELECT 'standard', 'Standard', 'Enhanced features for serious researchers', 9.99, 'month',
 '["Everything in Free", "Save up to 25 notebooks", "Submit up to 10 notebooks", "Advanced search features", "Email notifications", "Basic analytics"]',
 '{"savedNotebooks": 25, "submittedNotebooks": 10, "premiumContent": false}'
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE id = 'standard')
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
  description = EXCLUDED.description,
  price = EXCLUDED.price,
