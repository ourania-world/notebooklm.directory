-- Update subscription plans to allow unlimited submissions for all plans
...
-- Add popular column to subscription_plans if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'subscription_plans' AND column_name = 'popular'
  ) THEN
    ALTER TABLE subscription_plans ADD COLUMN popular boolean DEFAULT false;
  END IF;
END $$;

-- Update Standard plan to be popular
UPDATE subscription_plans
SET popular = true
WHERE id = 'standard';

-- Update Professional plan to not be popular
UPDATE subscription_plans
SET popular = false
